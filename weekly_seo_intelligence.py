...
def _img_tag(path: str, alt: str = "chart") -> str:
    """Return an img tag for a chart path, or empty string if path is falsy."""
    if not path or not os.path.exists(path):
        return f'<p style="{_MONO};font-size:10px;color:#94A3B8;">Chart image not found ({path}).</p>'
    return (
        f'<div style="width:100%;margin-bottom:24px;">'
        f'<img src="{_esc(str(path))}" alt="{_esc(alt)}" '
...
def _charts_section() -> str:
    """Embed charts generated by master_orchestrator.py."""
    charts = [
        ("charts/master_kpi_overview.png", "Weekly KPI Overview"),
        ("charts/master_traffic_search.png", "Traffic vs Search Demand"),
        ("charts/master_top_pages.png", "Top Landing Pages"),
        ("charts/master_kw_movement.png", "Keyword Ranking Movement"),
        ("charts/master_mobile_speed.png", "Mobile Performance Scores"),
    ]
    
    explanation = f'<p style="{_MONO};font-size:10px;color:#64748B;margin-bottom:16px;">\u24d8 <b>How to read:</b> Visual trends from GA4, GSC, and PageSpeed. Compare current (Blue/Teal) vs previous (Gray) performance.</p>'
    
    html = [explanation]
    for path, title in charts:
        html.append(f'<div style="{_MONO};font-size:9px;text-transform:uppercase;margin-bottom:8px;color:#525252;">{title}</div>')
        html.append(_img_tag(path, title))
...
def _action_table(actions: list) -> str:
    ...
    explanation = f'<p style="{_MONO};font-size:10px;color:#64748B;margin-bottom:12px;">\u24d8 <b>How to read:</b> Actions are prioritised by Business Value and Impact. High Impact/Low Effort items are the "quick wins".</p>'
...
def _anomaly_table(anomalies: list) -> str:
    ...
    explanation = f'<p style="{_MONO};font-size:10px;color:#64748B;margin-bottom:12px;">\u24d8 <b>How to read:</b> Significant deviations from historical patterns. Red severity indicates an urgent drop or spike needing investigation.</p>'
...
def _warnings_table(warnings: list) -> str:
    ...
    explanation = f'<p style="{_MONO};font-size:10px;color:#64748B;margin-bottom:12px;">\u24d8 <b>How to read:</b> Operational warnings regarding data availability and API connectivity. High severity items may result in incomplete reporting.</p>'
    return (
        explanation +
        f'<table style="width:100%;border-collapse:collapse;">'
        f'<thead><tr>{_th_row(headers)}</tr></thead>'
...
def _decay_table(decay_df) -> str:
    ...
    explanation = f'<p style="{_MONO};font-size:10px;color:#64748B;margin-bottom:12px;">\u24d8 <b>How to read:</b> Decay Score (0-100) identifies pages losing visibility. Higher scores (Red) indicate urgent need for content refresh.</p>'
...
def generate_html_report(
    ...
    body = (
        ...
        + _section("Performance Visuals",              _charts_section(),            "performance-visuals")
